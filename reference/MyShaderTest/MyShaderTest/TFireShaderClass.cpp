...
bool TFireShaderClass::Initialize(ID3D11Device* device,ID3D11DeviceContext* deviceContext, WCHAR* fxFilename, HWND hwnd)
{
	...
	m_fire = new TextureClass();
	result = m_fire->Initialize(device,L"fire01.dds");
	if(!result)
	{
		MessageBox(hwnd, L"Could not initialize the Texture object.", L"Error", MB_OK);
		return false;
	}
	m_noise = new TextureClass();
	result = m_noise->Initialize(device,L"noise01.dds");
	if(!result)
	{
		MessageBox(hwnd, L"Could not initialize the Texture object.", L"Error", MB_OK);
		return false;
	}
	m_fireTexture->SetResource(m_fire->GetTexture());
	m_noiseTexture->SetResource(m_noise->GetTexture());
