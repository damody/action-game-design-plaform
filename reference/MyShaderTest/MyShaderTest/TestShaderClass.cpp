...
TestShaderClass::TestShaderClass(void)
{
	m_layout = 0;
	m_PTech_Heroes = 0;
	m_Heroes_cLootAt = 0;
...
TestShaderClass::~TestShaderClass()
{
}
bool TestShaderClass::Initialize(ID3D11Device* device, WCHAR* fxFilename,HWND hwnd)
{
	bool result;
...
bool TestShaderClass::InitializeShader(ID3D11Device* device, WCHAR* fxFilename,HWND hwnd)
{
	ID3D10Blob* pCode = 0;
...
void TestShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, ID3D11ShaderResourceView* texture)
{
	...
	m_PTech_Heroes->GetPassByIndex(0)->Apply(0, deviceContext);
	deviceContext->Draw(indexCount, 0);
}

void TestShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, WCHAR* shaderFilename,HWND hwnd)
{
	char* compileErrors;
