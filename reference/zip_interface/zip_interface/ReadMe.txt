================================================================================
    MFC 程式庫 : zip_interface 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 zip_interface 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 zip_interface 應用程式的檔案。

zip_interface.vcxproj
    這是使用應用程式精靈所產生之 VC++ 專案的主要專案檔。 
    它包含產生檔案時的 Visual C++ 版本資訊，以及使用應用程式精靈產生檔案的
    過程中，選取的平台、組態和專案功能的相關資訊。

zip_interface.vcxproj.filters
...
zip_interface.h
    ...
    並宣告 Czip_interfaceApp 應用程式類別。

zip_interface.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 Czip_interfaceApp。
...
res\zip_interface.ico
    這個圖示檔是用來做為應用程式的圖示。這個圖示包含在主要資源檔
    zip_interface.rc 內。

res\zip_interface.rc2
...
zip_interfaceDlg.h、zip_interfaceDlg.cpp - 對話方塊
    這些檔案包含您的 Czip_interfaceDlg 類別。這個類別定義應用程式主
    對話方塊的行為。對話方塊的範本在 zip_interface.rc 內，您可以 
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 zip_interface.pch 的先行編譯標頭 (PCH) 檔，
