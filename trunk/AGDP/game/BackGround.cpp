...
void Background::BuildPoint()
{
    ...
    {
        if(g_Time % it->m_TimeLine < it->m_TimeStart)
        {
            continue;
        }

        if(g_Time % it->m_TimeLine > it->m_TimeEnd)
        {
            continue;
        }
...
bool Background::IsOpenUpDownBounding()
{
    // todo: 需要新增變數控制
    return false;
}

void Background::FixUpDownBounding(Vector3& pIn, const Vector3& vel)
{
    Vector3 cpos = pIn - vel;
    for(AxisAlignedBoxs::iterator it = m_SpaceBounding.begin(); it != m_SpaceBounding.end() ; it++)
    {
        Vector3 min = it->getMinimum();
        Vector3 max = it->getMaximum();
