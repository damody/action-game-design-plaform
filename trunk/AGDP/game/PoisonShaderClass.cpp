...
#include <iostream>
PoisonShaderClass::PoisonShaderClass() : EffectShaderClass()
{
}
...
void PoisonShaderClass::SetEffectVariableByName()
{
	m_poisonTexture = m_Effect->GetVariableByName( "poisonTexture" )->AsShaderResource();
...
{
	m_poison = Texture_Sptr( new Texture( L"Media\\poison.png" ) );
	m_noise = Texture_Sptr( new Texture( L"Media\\noise01.dds" ) );
	m_poisonTexture->SetResource( m_poison->texture );
	m_noiseTexture->SetResource( m_noise->texture );
}
void PoisonShaderClass::SetShaderParameters( D3DXVECTOR3 scrollSpeeds, D3DXVECTOR3 scales, D3DXVECTOR2 distortion1,
