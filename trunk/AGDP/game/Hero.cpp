...
ClipVertex Hero::GetPic() const
{
    return m_Pic;
}

int Hero::GetTextureID() const
{
    return m_Texture;
}
...
void Hero::NextFrame()
{
    ...
    {
        if(m_Vel.y < 10.0 && m_Vel.y > 5.0 && m_FrameID != 1)
        {
            m_FrameID = 1;
        }
        else if(m_Vel.y < 5.0 && m_Vel.y > -7.0 && m_FrameID != 2)
        {
            m_FrameID = 2;
        }
        else if(m_Vel.y < -7.0 && m_FrameID != 3)
        {
            m_FrameID = 3;
        }
        else
...
NextLoop:
    ...
    while(i != m_KeyQue.end())
    {
        ...
        else if(i->key == g_KeyMap.RightKey().keyDown)
        {
            dx = +1;
        }
        else if(i->key == g_KeyMap.UpKey().keyDown)
        {
            dz = +1;
        }
        else if(i->key == g_KeyMap.DownKey().keyDown)
        {
            dz = -1;
        }
...
bool Hero::ScanKeyQue()
{
    ...
    if(!m_KeyQue.empty())
    {
        ...
        for(unsigned int i = 0; i < hit.size(); i++)
        {
            ...
            if(nKey == 1 && g_KeyMap.isSKey(*rHit) && isKeyUsed(*rHit))
            {
                continue;
            }
            ...
            while(riKey != m_KeyQue.rend() && rHit + 1 != pHit && flag)
            {
                if(g_KeyMap.isKeyUp(*rHit))
                {
                    ...
                    if(!m_FaceSide)
                    {
                        if(rhd == g_KeyMap.LeftKey().keyDown)
                        {
                            rhd = g_KeyMap.RightKey().keyDown;
                        }
                        else if(rhd == g_KeyMap.RightKey().keyDown)
                        {
                            rhd = g_KeyMap.LeftKey().keyDown;
                        }
                    }
        ...
        if(kdn != 0)
        {
            ...
            for(i = m_KeyQue.begin(); i != m_KeyQue.end(); i++)
            {
                if(kdn == i->key)
                {
                    ...
                    if(k.key == g_KeyMap.LeftKey().keyUp)
                    {
                        d_run = -i->time;
                    }
                    else if(k.key == g_KeyMap.RightKey().keyUp)
                    {
                        d_run = i->time;
                    }
                    break;
...
void Hero::beAttack(const Attack* rAtk, const Hero* rHero, const Vector3& hitPos, bool rFace)
{
    if(rAtk->m_Kind == 0)                    //普通攻擊形式，套用 effect 擊中特效
    {
        ...
        wprintf(L"beAttack MaxHP=%d\tHP=%d\tMP=%d\tFall=%d\tfrontDef=%d\tbackDef=%d\n", m_MaxRecoverHP, m_HP, m_MP, m_Fall, m_FrontDefence, m_BackDefence);
        //effect
        if(m_HeroFunctionLua->HasValue("AddCondition"))
        {
...
const Vector3& Hero::Position() const
{
    return m_Position;
}
...
const Vector3& Hero::Velocity() const
{
    return m_Vel;
}
...
void Hero::ModifyFrontDefence(int delta)
{
    m_FrontDefence += delta;
}
void Hero::ModifyBackDefence(int delta)
{
    m_BackDefence += delta;
}
void Hero::ChangeTeam(int newTeam)
{
    m_Team = newTeam, m_Record->team = newTeam;
}
