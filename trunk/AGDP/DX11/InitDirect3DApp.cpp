...
void InitDirect3DApp::UpdateScene(float dt)
{
    ...
    if(g_TestViewEffect)
    {
        ...
        {
            g_EffectManager->Update(m_RenderTargetView);
        }
...
void InitDirect3DApp::OnResize()
{
    ...
    if(g_EffectManager != NULL)
    {
        g_EffectManager->OnResize(m_ClientWidth, m_ClientHeight);
    }
...
void InitDirect3DApp::buildPoint()
{
    ...
    if(!g_ObjectManager.ObjectEmpty())
    {
        for(Objects::iterator it = g_ObjectManager.ObjectVectorBegin(); it != g_ObjectManager.ObjectVectorEnd(); it++)
        {
            PolygonVerteices bvs = (*it)->GetPolygonVerteices();
            m_PolygonVerteices.insert(m_PolygonVerteices.end(), bvs.begin(), bvs.end());
        }
