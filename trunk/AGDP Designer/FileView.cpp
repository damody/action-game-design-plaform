...
int CFileView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndToolBar.SetPaneStyle(m_wndToolBar.GetPaneStyle() & ~(CBRS_GRIPPER | CBRS_SIZE_DYNAMIC |
...
void CFileView::AdjustLayout()
{
	...
	m_wndToolBar.SetWindowPos(NULL, rectClient.left, rectClient.top, rectClient.Width(), cyTlb,
							  SWP_NOACTIVATE | SWP_NOZORDER);
	m_wndFileView.SetWindowPos(NULL, rectClient.left + 1, rectClient.top + cyTlb + 1,
							   rectClient.Width() - 2, rectClient.Height() - cyTlb - 2, SWP_NOACTIVATE | SWP_NOZORDER);
}
...
//          {
			...
			if (g_HeroInfoMap.find(hero->m_Name) == g_HeroInfoMap.end())
			{
				...
				for (unsigned int i = 0; i < hero->m_PictureDatas.size(); i++)
				{
					...
					hero->m_PictureDatas[i].m_TextureID = g_TextureManagerFrame->AddTexture(
							hero->m_PictureDatas[i].m_Path);
...
void CFileView::OnChangeVisualStyle()
{
	...
	m_wndToolBar.LoadBitmap(theApp.m_bHiColorIcons ? IDB_EXPLORER_24 : IDR_EXPLORER, 0, 0,
							TRUE /* 鎖定 */);
	...
	if (it != m_HeroInfoMap.end())
	{
		...
		if (((CMainFrame*)this->GetParentFrame())->NewHeroViews(it->second))
		{
			for (unsigned int i = 0; i < it->second->m_PictureDatas.size(); i++)
			{
				...
				it->second->m_PictureDatas[i].m_TextureID = g_TextureManagerFrame->AddTexture(
							it->second ->m_PictureDatas[i].m_Path);
				((CMainFrame*)this->GetParentFrame())->OpenPictureView(str, &it->second->m_PictureDatas[i], i,
						it->second);
			}
		}
		((CMainFrame*)this->GetParentFrame())->m_wndClassView.Refresh();
...
void CFileView::OnPicturedataAdd()
{
	// TODO: 在此加入您的命令處理常式程式碼
	CFileDialog dlgFile(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
						_T("Data Files (*.bmp;*.png;*.jpg; *.gif)|*.bmp; *.png; *.jpg; *.gif;||"), NULL, 0);
	if (dlgFile.DoModal() == IDOK)
	{
		HeroInfoHMap::iterator it_Hero = m_HeroInfoMap.find(hHero_Select);
		if (it_Hero == m_HeroInfoMap.end())
		{
			return;
		}
		PictureData pic;
		...
		((CMainFrame*)this->GetParentFrame())->OpenPictureView(dlgFile.GetFileName(),
				&it_Hero->second->m_PictureDatas.back(), it_Hero->second->m_PictureDatas.size() - 1,
				it_Hero->second);
	}
}
...
void CFileView::LoadData()
{
	...
	for (unsigned int idx = 0; idx < heroInfos.size(); idx++)
	{
		...
		HTREEITEM heroH = m_wndFileView.InsertItem(CString(heroInfos[idx]->m_Name.c_str()), 2, 2,
						  hResourceHeroDoc);
