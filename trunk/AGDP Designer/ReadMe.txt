================================================================================
    MFC 程式庫 : AGDP Designer 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 AGDP Designer 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 AGDP Designer 應用程式的檔案。

AGDP Designer.vcxproj
    這是使用應用程式精靈所產生之 VC++ 專案的主要專案檔。 
    它包含產生檔案時的 Visual C++ 版本資訊，以及使用應用程式精靈產生檔案的
    過程中，選取的平台、組態和專案功能的相關資訊。

AGDP Designer.vcxproj.filters
...
AGDP Designer.h
    ...
    並宣告 CAGDPDesignerApp 應用程式類別。

AGDP Designer.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CAGDPDesignerApp。
...
res\AGDP Designer.ico
    這個圖示檔是用來做為應用程式的圖示。這個圖示包含在主要資源檔
    AGDP Designer.rc 內。

res\AGDPDesigner.rc2
...
AGDP DesignerDoc.h、AGDP DesignerDoc.cpp - 文件
    這些檔案包含您的 CAGDPDesignerDoc 類別。編輯這些檔案，即可新增您的特
    殊文件資料，並實作檔案儲存和載入 (透過 CAGDPDesignerDoc::Serialize)。

AGDP DesignerView.h、AGDP DesignerView.cpp - 文件的檢視
    這些檔案包含您的 CAGDPDesignerView 類別。
    CAGDPDesignerView 物件是用來檢視 CAGDPDesignerDoc 物件。

res\AGDP DesignerDoc.ico
    這個圖示檔是用來做為 CAGDPDesignerDoc 類別之 MDI 子視窗的圖示。
    這個圖示是包含在主要資源檔 AGDP Designer.rc 內。
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 AGDP Designer.pch 的先行編譯標頭 (PCH) 檔，
